...
exports.Sshtate = exports.default = void 0;
...
var get = function get(root, key) {
  return key.split('.').reduce(function (acc, current) {
    return acc[current];
  }, root);
};
/**
...
var set = function set(root, key, val) {
...
var ev = function ev(storeEl, unique, key, cb) {
  var callback = function callback(_ref) {
    var detail = _ref.detail;
    return cb(detail.new, detail.previous);
  };

  if (cb) Listeners[unique] = callback;
  storeEl[!cb ? 'removeEventListener' : 'addEventListener']("Sshtate.".concat(key), !cb ? Listeners[unique] : callback);
...
/*#__PURE__*/
function () {
  function Sshtate(initialState) {
    _classCallCheck(this, Sshtate);

    storeEl = document.createElement('meta');
    state = initialState || {};
  }

  _createClass(Sshtate, [{
    key: "setState",
    value: function setState(key, val) {
      var currentState = this.getState(key);

      if (JSON.stringify(val) !== JSON.stringify(currentState)) {
        set(state, key, val);
        storeEl.dispatchEvent(new CustomEvent("Sshtate.".concat(key), {
          bubbles: true,
          detail: {
            new: val,
    ...
    key: "subscribe",
    value: function subscribe(unique, key, cb) {
      ev(storeEl, unique, key, cb);
    }
  }, {
    key: "unsubscribe",
    value: function unsubscribe(unique, key, cb) {
...
exports.Sshtate = Sshtate;
var _default = Sshtate;
...
var logChange = function logChange(prefix, data) {
  return console.log(prefix, data);
};

Food.subscribe('subscriptionId1', 'bread.loaf', function (data) {
  return logChange('subscriptionId1:', data);
});
Food.subscribe('subscriptionId2', 'bread.loaf', function (data) {
  return logChange('subscriptionId2:', data);
});
...
//# sourceMappingURL=/sshtate.e31bb0bc.js.map